...
local function checkOwner(updatedWindow)
    ...
    elseif not updatedWindow.params.ID or not stringStartsWith(updatedWindow.params.ID, "com.lab126.") then
        -- assume current applciation but log an error
        llog.info("WindowManager", "checkOwnerFail", "ID=updatedWindow.params.ID", "dialog is incorrectly named")
        local activeApp = getActiveApplicationWindow()
        updatedWindow.owner = activeApp and activeApp.params.ID
    else 
        log("dialog owner is " .. updatedWindow.params.ID)
        updatedWindow.owner = updatedWindow.params.ID
